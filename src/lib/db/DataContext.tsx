...
import { initReviewSchedule, advanceReviewSchedule } from "@/lib/revision/spacedRepetition";
...
interface DataContextValue {
  ...
  markReviewed: (topicId: string) => Promise<void>;
...
export function DataProvider({ children }: { children: ReactNode }) {
  ...
  const completedTopicsCount = useMemo(
    () => Object.values(topicStates).filter((s) => s.isCompleted).length,
    [topicStates]
  );

  // Schedule / reschedule the daily study reminder whenever the setting (ya progress/exam date) changes
  useEffect(() => {
    if (!ready) return;
    if (settings.notificationsEnabled) {
      scheduleDailyReminder(settings, { completedTopics: completedTopicsCount, totalTopics: TOTAL_TOPICS });
    } else {
      cancelDailyReminder();
    }
  }, [ready, settings.notificationsEnabled, settings.notificationTime, settings.examDate, completedTopicsCount]);
  ...
  const toggleComplete = useCallback(async (topicId: string, estimatedMinutes: number) => {
    ...
    const next: TopicState = {
      ...
      // Pehli baar complete hone par spaced-repetition schedule shuru karo.
      ...(willComplete && prev.reviewStage == null ? initReviewSchedule() : {}),
    };
  ...
  /** Spaced-repetition: user ne is topic ka revision aaj complete kar liya — agla interval schedule karo. */
  const markReviewed = useCallback(async (topicId: string) => {
    const prev = topicStates[topicId] ?? { topicId, isCompleted: false };
    const schedule = advanceReviewSchedule(prev.reviewStage);
    await persistTopic({ ...prev, ...schedule });
  }, [topicStates, persistTopic]);
  ...
  const value = useMemo<DataContextValue>(() => ({
    ...
    setDifficulty, setNotes, markReviewed, updateSettings, resetAll, exportJSON, importJSON,
